/**
 * @brief Tensor-core Q4_K x Q8_1 mul-mat-vec for batch=1 decode.
 *
 ...
 *   A: 16 weight rows x 32 K (s8 dequant from 4-bit), per IMMA
 *   B: 32 K of q8_1 input (broadcast across 8 N - N>0 wasted)
 *   D: 16 x 8 s32 (only col 0 used)
 *
 ...
 * selected by ip (low/high). scales[12] holds 8x(6-bit scale, 6-bit min).
 *
 * The dp4a form of the same dot product is `vec_dot_q4_K_q8_1` in `gguf.cuh`.
 */
#include <cuda.h>
#include <cuda_runtime.h>
#include <cuda_fp16.h>
#include <cstdint>
// The block layouts, `QK_K`/`QK8_1` and the six-bit scale unpack come from `gguf.cuh`, which
// states them once for every kernel that reads these bytes. `-I cuda/moe` resolves it (see
// build.rs); this file is compiled by nvcc, so the include is the sharing mechanism here.
#include "gguf.cuh"

namespace loken_q4k_imma_ns {

// One block = one warp = 16 weight rows x 1 input col.
// gridDim.x = ceil(nrows_x / 16), blockDim.x = 32.
__global__ void q4k_mmvq_imma_kernel(
    const block_q4_K * __restrict__ vx,   // [nrows_x x num_super]
    const block_q8_1 * __restrict__ vy,   // [num_super x 8]
    ...
    for (int isb = 0; isb < num_super; ++isb) {
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            get_scale_min_k4(s, x_g->scales,  sc_g,  m_g);
            get_scale_min_k4(s, x_g8->scales, sc_g8, m_g8);
...
// NOTE: keep this symbol UNPREFIXED. The generic (host-provided)
// `extern "C" fn q4k_mmvq_imma` links against this exact name; the substrate
// must stay free of loken-specific symbols. Other relocated kernels are
// server-internal and carry the `loken_` prefix - this one is shared.
extern "C" void q4k_mmvq_imma(
    const void * vx,           // device ptr [nrows_x x num_super x sizeof(block_q4_K)]
    const void * vy,           // device ptr [num_super x 8 x sizeof(block_q8_1)]
    void * dst,                // device ptr [nrows_x] f32
    int ncols_x,               // K dim, must be multiple of 256
    int nrows_x,
    cudaStream_t stream
) {
    const int num_super = ncols_x / QK_K;
    const int row_groups = (nrows_x + 15) / 16;
    dim3 grid(row_groups, 1, 1);
    dim3 block(32, 1, 1);
    loken_q4k_imma_ns::q4k_mmvq_imma_kernel<<<grid, block, 0, stream>>>(
        (const block_q4_K *)vx,
        (const block_q8_1 *)vy,
        (float *)dst,
        num_super,
