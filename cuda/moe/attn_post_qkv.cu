 ...
 *
 * A per-op composition produces 5+ kernel launches per layer for
...
// Partial-RoPE variant of attn_post_qkv_decode_qf32_kernel (WithNorm=0).
// Rotates only the first `rope_dim` of `hd` elements per head; the
// remaining `hd - rope_dim` pass through unchanged. Phi2 / phi3 / GPT-NeoX
// use this - rope_dim is typically half of head_dim. The cos/sin tables
// must be sized [max_seq, rope_dim/2] (stride = rope_dim/2 per position).
//
// Block(0): Q heads. Q is written F32.
// Block(1): K heads. K is written as `T` dtype.
// Block(2): V heads (just pass-through cast, no rotation).
template <typename T, int RopeStyle>
__global__ void attn_post_qkv_decode_qf32_partial_rope_kernel(
    const float* __restrict__ qkv,
    const T*     __restrict__ rope_cos,
    const T*     __restrict__ rope_sin,
    ...
    int n_q, int n_kv, int hd, int rope_dim,
    int rope_pos,
    float q_scale
) {
    const int kind = blockIdx.x;
    ...
    if (tid < rope_dim) {
        ...
        if constexpr (RopeStyle == 0) {
            if (tid < half) {
                const float cur   = shared_v[tid];
                const float other = shared_v[tid + half];
                const float c = to_float<T>(rope_cos[row_base + tid]);
                const float s = to_float<T>(rope_sin[row_base + tid]);
                v = cur * c - other * s;
            } else {
                const float cur   = shared_v[tid];
                const float other = shared_v[tid - half];
                const int rope_idx = tid - half;
                const float c = to_float<T>(rope_cos[row_base + rope_idx]);
                const float s = to_float<T>(rope_sin[row_base + rope_idx]);
                v = cur * c + other * s;
            }
        } else {
            const int pair = (tid & 1) ? tid - 1 : tid + 1;
            const float cur   = shared_v[tid];
            const float other = shared_v[pair];
            const int rope_idx = tid >> 1;
            const float c = to_float<T>(rope_cos[row_base + rope_idx]);
            const float s = to_float<T>(rope_sin[row_base + rope_idx]);
            if ((tid & 1) == 0) v = cur * c - other * s;
            else                v = other * s + cur * c;
    ...
    // tid >= rope_dim: v stays unchanged (pass through).
...
extern "C" void loken_attn_post_qkv_decode(
    const float* qkv,
...
#define LAUNCH(T, STYLE) \
    loken_ll_apq::attn_post_qkv_decode_kernel<T, STYLE> \
...
extern "C" void loken_attn_post_qkv_decode_qf32(
    const float* qkv,
    const float* q_norm_w,
    const float* k_norm_w,
    const void*  rope_cos,
...
#define LAUNCH_QF32(T, STYLE) \
    loken_ll_apq::attn_post_qkv_decode_qf32_kernel<T, STYLE> \
        <<<grid, block, smem_bytes, stream>>>( \
            qkv, q_norm_w, k_norm_w, \
            (const T*)rope_cos, (const T*)rope_sin, \
            q_out, (T*)k_out, v_out, \
...
// variant.
extern "C" void loken_attn_post_qkv_decode_qf32_no_norm(
    const float* qkv,
    const void*  rope_cos,
    const void*  rope_sin,
    float*       q_out,
...
#define LAUNCH_QF32_NN(T, STYLE) \
    loken_ll_apq::attn_post_qkv_decode_qf32_kernel<T, STYLE, 0> \
        <<<grid, block, smem_bytes, stream>>>( \
...
// Partial-RoPE variant of attn_post_qkv_decode_qf32_no_norm. When
// rope_dim < hd, only the first rope_dim of hd elements get rotated;
// the rest pass through unchanged. Used by phi2/phi3-style models (e.g.
// rope_dim=32, head_dim=64).
extern "C" void loken_attn_post_qkv_decode_qf32_no_norm_partial_rope(
    const float* qkv,
    const void*  rope_cos,
    const void*  rope_sin,
    float*       q_out,
    ...
    dim3 grid(3, max(n_q, n_kv), 1);
    dim3 block(hd, 1, 1);
    size_t smem_bytes = hd * sizeof(float);

#define LAUNCH_QF32_PR(T, STYLE) \
    loken_ll_apq::attn_post_qkv_decode_qf32_partial_rope_kernel<T, STYLE> \
        <<<grid, block, smem_bytes, stream>>>( \
            qkv, (const T*)rope_cos, (const T*)rope_sin, \
            q_out, (T*)k_out, v_out, \
            n_q, n_kv, hd, rope_dim, rope_pos, q_scale);

    if (dtype == 0) {
        if (rope_style == 0) { LAUNCH_QF32_PR(__half, 0); }
        else                 { LAUNCH_QF32_PR(__half, 1); }
    } else if (dtype == 1) {
#ifndef NO_BF16_KERNEL
        if (rope_style == 0) { LAUNCH_QF32_PR(__nv_bfloat16, 0); }
        else                 { LAUNCH_QF32_PR(__nv_bfloat16, 1); }
#endif
    } else {
        if (rope_style == 0) { LAUNCH_QF32_PR(float, 0); }
        else                 { LAUNCH_QF32_PR(float, 1); }
    }
#undef LAUNCH_QF32_PR
}
