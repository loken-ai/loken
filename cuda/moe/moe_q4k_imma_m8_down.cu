/**
 * @file loken_moe_q4k_imma_m8_down.cu
 *
 ...
 *   - Inputs pre-quantized to Q8_1 by the orchestrator: one Q8_1 row per
 *     routed pair, NOT per real token - down inputs are per-pair-distinct
 *     after gate||up.GELU.mul. The row is addressed by the pair's SLOT,
 *     sorted_token_ids[pair], which is where every producer writes it.
 *
 * Grid:
...
namespace loken_moe_q4k_imma_m8_down_ns {
...
extern "C" __global__ void loken_moe_q4k_imma_m8_down_kernel(
    ...
    using namespace loken_moe_q4k_imma_m8_down_ns;
    ...
    // The distinct experts this tile serves. A tile holds IDN_N pairs and each may name an
    // expert of its own, so the list is IDN_N long: anything shorter drops the pairs whose
    // expert appears late in a crowded tile.
    int experts_in_block[IDN_N];
    #pragma unroll
    for (int i = 0; i < IDN_N; ++i) experts_in_block[i] = -1;
    int num_block_experts = 0;
    #pragma unroll
    for (int p = 0; p < IDN_N; ++p) {
    ...
    const block_q4_K * w_expert =
        (const block_q4_K *) down_w
        + (size_t)block_expert * (size_t)hidden * num_super;

    // Down inputs are per-pair (not per-real-token like gate||up), and the row is the pair's
    // slot rather than its sorted position: the two differ whenever topk > 1.
    const block_q8_1 * y_pair = nullptr;
    if (v_pair_e) {
        y_pair = (const block_q8_1 *) inputs_q81
               + (size_t)my_pair_tok * num_super * 8;
    }

    float out_0 = 0.f, out_1 = 0.f, out_2 = 0.f, out_3 = 0.f;

    for (int isb = 0; isb < num_super; ++isb) {
        const block_q4_K * wa_sb = va ? w_expert + (size_t)row_a * num_super + isb : nullptr;
        const block_q4_K * wb_sb = vb ? w_expert + (size_t)row_b * num_super + isb : nullptr;
        ...
        for (int s = 0; s < 8; ++s) {
            uint8_t sc, m;
            if (wa_sb) { get_scale_min_k4(s, wa_sb->scales, sc, m); dall_sc_a[s] = dall_a * (float)sc; dmin_m_a[s] = dmin_a * (float)m; }
            else       { dall_sc_a[s] = 0.f; dmin_m_a[s] = 0.f; }
            if (wb_sb) { get_scale_min_k4(s, wb_sb->scales, sc, m); dall_sc_b[s] = dall_b * (float)sc; dmin_m_b[s] = dmin_b * (float)m; }
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            // Vectorize 4-byte uint32 reads into uint2 (8-byte). qs at
            // byte offset 16 (after dm + scales[12]), 8-byte aligned.
            ...
            const block_q8_1 * yb_my = y_pair ? y_pair + isb * 8 + s : nullptr;
...
extern "C" void loken_moe_q4k_imma_m8_down(
    const void * down_w,
    ...
    dim3 grid((hidden + IDN_M - 1) / IDN_M,
              ...
              1);
    dim3 blk(32, 1, 1);
    loken_moe_q4k_imma_m8_down_kernel<<<grid, blk, 0, stream>>>(
