 ...
 * path did a clone_dtod + slice_set + memcpy_dtod (3 launches x 2
 * residuals per layer = 6/layer x 48 layers ≈ 288 extra launches/token).
 *
...
extern "C" void loken_kv_residual_scatter_f16(
    ...
    loken_ll_kv_scatter::kv_residual_scatter_kernel<<<grid, block, 0, stream>>>(
        (const __half *)src,
        (__half *)dst,
        n_kv,
        head_dim,
...
extern "C" void loken_kv_residual_scatter_f16_dev_slot(
    const void  * src,
    void        * dst,
    ...
    const int block = 128;
    const int grid  = (total + block - 1) / block;
    loken_ll_kv_scatter::kv_residual_scatter_kernel_dev_slot<<<grid, block, 0, stream>>>(
        (const __half *)src,
        (__half *)dst,
...
extern "C" void loken_flush_k_residual_q4_dev_pos(
    ...
    dim3 block(32, 1, 1);
    loken_ll_kv_scatter::flush_k_residual_q4_dev_pos_kernel<<<grid, block, 0, stream>>>(
...
extern "C" void loken_q4_v_scatter_bytes_dev_pos(
    ...
    loken_ll_kv_scatter::q4_v_scatter_bytes_kernel<<<grid, block, 0, stream>>>(
        (const uchar4 *)src,
        (uchar4 *)dst,
        (const int32_t *)pos_dev,
        token_words
