 ...
 *
 * Saves 1 launch per layer per token (~5-7 µs CPU overhead x 48 layers
 * x decode tok rate). On qwen3-coder this is ~240 µs/token = ~3%.
 *
 * Single-block design: hidden <= 8192 fits in 1024 threads x 8 elements.
...
template <int block_size>
__global__ void loken_add_rms_norm_kernel(
...
extern "C" void loken_add_rms_norm(
    ...
    ll_addnorm::loken_add_rms_norm_kernel<block_size>
