 ...
 * blockDim = 32 (1 warp)
 *
 * Hypothesis: M=1 broadcast IMMA wastes 8x compute (mma produces 16x8
 * but only col 0 is used). M=8 batched IMMA recovers the 8x via real
 * column outputs.
 */
#include "gguf.cuh"
...
namespace loken_moe_q4k_imma_m8_ns {
...
extern "C" __global__ void loken_moe_q4k_imma_m8_kernel(
    ...
    using namespace loken_moe_q4k_imma_m8_ns;
    ...
    for (int isb = 0; isb < num_super; ++isb) {
        const block_q4_K * gwa = va ? w_expert + (size_t)row_a    * num_super + isb : nullptr;
        const block_q4_K * gwb = vb ? w_expert + (size_t)row_b    * num_super + isb : nullptr;
        const block_q4_K * uwa = va ? w_expert + (size_t)row_a_up * num_super + isb : nullptr;
        const block_q4_K * uwb = vb ? w_expert + (size_t)row_b_up * num_super + isb : nullptr;
        ...
        for (int s = 0; s < 8; ++s) {
            uint8_t sc, m;
            if (gwa) { get_scale_min_k4(s, gwa->scales, sc, m); g_dall_sc_a[s] = g_dall_a * (float)sc; g_dmin_m_a[s]  = g_dmin_a * (float)m; }
            else     { g_dall_sc_a[s] = 0.f; g_dmin_m_a[s] = 0.f; }
            if (gwb) { get_scale_min_k4(s, gwb->scales, sc, m); g_dall_sc_b[s] = g_dall_b * (float)sc; g_dmin_m_b[s]  = g_dmin_b * (float)m; }
            else     { g_dall_sc_b[s] = 0.f; g_dmin_m_b[s] = 0.f; }
            if (uwa) { get_scale_min_k4(s, uwa->scales, sc, m); u_dall_sc_a[s] = u_dall_a * (float)sc; u_dmin_m_a[s]  = u_dmin_a * (float)m; }
            else     { u_dall_sc_a[s] = 0.f; u_dmin_m_a[s] = 0.f; }
            if (uwb) { get_scale_min_k4(s, uwb->scales, sc, m); u_dall_sc_b[s] = u_dall_b * (float)sc; u_dmin_m_b[s]  = u_dmin_b * (float)m; }
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            // GATE A fragments.
            // Vectorize the 4 weight nibble reads into uint2 (8 bytes each).
            // Each pair {qs_off+0, qs_off+4} fits in one uint2 load.
            ...
            const block_q8_1 * yb_my = y_expert
                ? y_expert + isb * 8 + s
                : nullptr;
            // Q8_1 qs is at byte offset 4 within the struct (after __half2
            // ds), so uint2 (8-byte aligned) loads on qs+8*tj are
            // misaligned. Keep the 2 separate uint reads.
    ...
    // Each lane (g) owns ONE input pair (pair_base + g). The MMA output
    // for THIS lane's input pair is in g_acc[*] slots that correspond
    // to in_row = 2tj+0 (using g=2tj+0's data via shfl) and 2tj+1
    // (g=2tj+1). We need the EXPERT of each output's input pair to
    // gate writes. Pre-shfl my_expert from the source lane.
...
extern "C" void loken_moe_q4k_imma_m8_gate_up(
    ...
    dim3 grid((N      + IM8_M - 1) / IM8_M,
              ...
              1);
    dim3 blk(32, 1, 1);
    loken_moe_q4k_imma_m8_kernel<<<grid, blk, 0, stream>>>(
